...
To make this critique concrete, we conduct a \emph{prior-sweep
experiment}: the linguistic feature vectors for P, D, and JE
(reconstructed from MLE-MVN archaism scores; see Methods) are held
fixed, while the Gaussian prior $\mathcal{N}(\mu_0,\sigma_0^2)$ on the
date grid is varied across six scenarios, ranging from an uninformative
flat (data-only) prior through the agnostic Mode-B prior
$\mathcal{N}(575,400^2)$, a tight scholarly prior
$\mathcal{N}(625,100^2)$ reflecting the Josianic--Persian consensus, and
two explicitly traditionalist assumptions: an early-monarchy prior
...
all rows; only the assumed prior changes.
...
the Persian period (539--332~BCE).  The Mosaic prior cannot move any
source into pre-exilic territory because the linguistic signal is too
strongly inconsistent with a pre-exilic date.  For P, which carries
the strongest data signal ($n = 53{,}645$~words), the total
prior-induced shift across the full range from flat to Mosaic is
87~yr, confirming the data-driven classification from the prior
sensitivity analysis.

The practical conclusion is direct.  An investigator who imposes
Mosaic-authorship assumptions on our uncontaminated feature vectors
still recovers Persian-period dates.  Pre-exilic Torah dates are
obtainable only by one of two routes: (a)~fixing the prior so
extremely that it overwhelms a contradictory data signal, or
(b)~including the Torah in the training corpus so that ``Classical
Biblical Hebrew'' features are extracted partly from passages assumed
to be early --- the traditional procedure, and precisely the vicious
circle identified above.  Our model uses neither route: the training
corpus excludes Torah and DH texts entirely, and the prior is set
before fitting.  The circularity is not a philosophical abstraction
but a quantifiable discrepancy between prior assumptions and
linguistic evidence.
...
three Torah source composites include substantial legal sections (apodictic
and casuistic law, cultic regulations, purity codes) whose register may
differ from narrative prose for reasons unrelated to date.  Features with
strong genre bias (genre ratio $\gamma_j > 1.5$; S1~Table) are excluded
from the model feature set, and the archaism diagnostic is restricted to
the resistant-model subset that is least genre-sensitive.  Nevertheless,
genre confounds not captured by the three-level register classification
cannot be ruled out.  The results for D and P should be interpreted in
light of this gap; the consistent post-exilic direction across both models
and across multiple feature exclusion schemes (S4~Table) provides some
assurance that the genre gap is not driving the headline finding, but it
remains a genuine limitation.

\textbf{Source-decomposition uncertainty.}  The documentary source
composites follow the standard critical partition of Baden~\cite{Baden2012}.  Any
mis-assigned verses alter the feature vector and through it the MAP date.
The robustness check in S4~Table suggests dates are stable to
...
by a larger amount.  Users who adopt a different documentary partition
...
already mild extrapolations; dates for texts genuinely composed before
760~BCE or after 167~BCE have no calibration constraint and should be
treated accordingly.  The date grid imposes hard boundaries at
50~BCE and 1200~BCE; any composition outside this range would be pinned
to the nearest edge.
...
coverage.  Any quoted credible interval should be understood against this
overconfidence background; for policy-relevant or high-stakes applications,
the calibration-corrected interval should be used in preference to the
nominal one.
...
Within these limits the computational results find strong support in an
independent scholarly tradition.  The Copenhagen school of minimalist
biblical criticism has argued on historical-critical and philological
grounds that most Pentateuchal and historiographic texts reached their
canonical written form during the Persian or Hellenistic
period~\cite{Thompson1999,Lemche2008,Davies1992}.  Lemche identifies the
Persian and Hellenistic periods as the single ``mental matrix'' that best
explains the literary character of most Old Testament
texts~\cite{Lemche2008}, a date range that encompasses our MAP estimates
for all three sources.  Independent archaeological evidence reinforces
this picture: Adler's systematic survey of the material record finds no
evidence for widespread observance of Torah regulations --- dietary laws,
miqvaot, stone-vessel purity practices --- before the mid-Hasmonean period
(mid-2nd century~BCE)~\cite{Adler2022}, suggesting that the Torah had not
yet achieved canonical authority among ordinary Judeans before that time.
Barnea's epigraphic and historical analysis of Yahwism under the
Achaemenid empire similarly locates the crystallization of distinctively
Jewish practice in the Persian period, with probable Zoroastrian influence
on rituals the Torah legislates~\cite{Barnea2026}.  At the level of the
Torah's central narrative figure, Adair traces the Moses tradition to a
4th-century~BCE literary construction drawing on Egyptian folk-heroic
conventions, placing the narrative scaffolding of the Pentateuch itself in
the late Persian or early Hellenistic period~\cite{Adair2025}.

Taken together, these independent lines of evidence --- historical
criticism, material archaeology, epigraphy, and comparative mythology ---
converge on the same chronological window that the present linguistic model
returns under agnostic prior assumptions.  Our results are best understood
not as an outlier relative to the total scholarly landscape, but as
independent quantitative confirmation, from morphosyntactic and lexical
feature distributions alone, of a conclusion that has been advanced through
multiple non-linguistic methods.  The convergence of entirely different
evidentiary approaches on the same date range constitutes a consilience
that strengthens each individual line of argument.  Precisely because our
method makes no use of historical, archaeological, or narrative-critical
evidence in its inference --- features are extracted from the morphological
corpus and dates are assigned by posterior maximization over an agnostic
prior --- agreement with those independent traditions cannot be an artifact
...
tags and can be verified without running the model.}
\label{tab:s3_features}
\begin{tabular}{llp{5.5cm}rp{3.8cm}}
\hline
\textbf{Source} & \textbf{Direction} & \textbf{Feature} & \textbf{Score} & \textbf{Note} \\
\hline
\multicolumn{5}{l}{\textit{D source composite (Deuteronomist; MAP$_{\text{HB-VI}}$ = 698--737~BCE, MAP$_{\text{MVN}}$ = 292~BCE)}} \\
...
\hline
\multicolumn{5}{l}{\textit{P source composite (Priestly; MAP$_{\text{HB-VI}}$ = 402--406~BCE, data-driven Class~D, $|\Delta_{AB}|=4$~yr)}} \\
...
\hline
\multicolumn{5}{l}{\textit{JE source composite (Jahwist-Elohist; MAP$_{\text{HB-VI}}$ $\approx$ 435~BCE, prior-dominated Class~P, $|\Delta_{AB}|=172$~yr)}} \\
...
JE (Jahwist-Elohist)  & 442 & 456 & $+14$ & 456 & Persian--early Hellenistic \\
\hline
\multicolumn{6}{p{14.5cm}}{\small Baseline dates use agnostic prior (Mode~B, $\mathcal{N}(575,400^2)$~BCE) to isolate
the data signal from prior assumptions.  Positive $\Delta$ indicates shift toward more
recent date after feature exclusion.  ``Persian period'' = 539--332~BCE; ``Persian--early
Hellenistic'' = 539--300~BCE.  The 74~yr shift for D after excluding \textit{asher} confirms
